...
### 0. テーブルが作成されているか確認（重要！）

まず、`stores`テーブルが正しく作成されているか確認します。

1. [Supabase](https://supabase.com) にログイン
2. プロジェクトを選択
3. 左メニューから「Table Editor」を選択
4. `stores`テーブルが存在するか確認
5. 存在しない場合、または`address`カラムがない場合は、以下の手順でテーブルを作成してください
...
4. 「Run」ボタンをクリック
5. 完了後、「Table Editor」でテーブルが作成されたことを確認
